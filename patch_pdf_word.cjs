...
// replace the clientPdfToWord function
const newPdfToWord = `async function clientPdfToWord(file: File, settings: Record<string, any>): Promise<ProcessingResult> {
  ...
  for (let i = 1; i <= pdfDoc.numPages; i++) {
    ...
    if (mode === 'ocr') {
       // OCR Mode: Render page as image, run tesseract, and just place it on the page.
      ...
      if (items.length === 0) {
         paragraphs.push(
           new Paragraph({
              children: [
                new TextRun({ text: "[Image Page - Select OCR mode to attempt text recognition.]", color: "888888" })
              ]
           })
         );
      } else {
        ...
        for (const line of lines) {
          ...
          // pdfjs transform is [scaleX, skewY, skewX, scaleY, x, y]
          // y is usually from bottom in PDF coordinates
          const pdfY = firstItem.transform[5];
          const pdfX = firstItem.transform[4];
          
          // Estimate font size (scaleY or scaleX)
          const fontSizePt = Math.max(8, Math.abs(firstItem.transform[3] || firstItem.transform[0] || 12));
          
          // Approximate top Y from baseline
          ...
          paragraphs.push(new Paragraph({
            children: [new TextRun({ text: lineText, size: Math.round(fontSizePt * 2) })],
            frame: {
              position: {
                x: xTwips,
                y: yTwips,
              ...
              anchor: {
                horizontal: 'page', // FrameAnchorType.PAGE
                vertical: 'page', // FrameAnchorType.PAGE
              },
              wrap: {
                type: 0, // TextWrappingType.NONE
              }
            }
          }));
        }
